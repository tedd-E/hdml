...
def add_args(parser):
    ...
    parser.add_argument('--data_size_per_client', type=int, default=1000,
                        help='input batch size for training (default: 64)')
...
if __name__ == '__main__':
    ...
    test_batch_selection = [1,34,62,22,65,55,98,35,77,9]
    ...
    #projector.load_state_dict(torch.load("encoder.ckpt"))
    ...
    for c in range(args.comm_round):
        ...
        print("+++++++++++++++++++Aggregating")
        global_model_aggregator.aggregate(local_model)
        print("+++++++++++++++++++distribute")
        for i in range(len(clients)):
            clients[i].set_model_params(global_model.get_model_params())
        print("+++++++++++++++++++Test")
        if c % args.frequency_of_the_test == 0:
            acc_over_rounds.append(global_model_aggregator.test_on_server_for_all_clients(test_batch_selection))
        else:
            print("skipped")
        print("===================RoundComplete====================")
